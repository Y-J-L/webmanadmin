...
//         ],
        ...
        if (!empty($data) && is_array($data)) {
            ...
            foreach ($data as $key => $rows) {
                ...
                foreach ($rows as $keyName => $value) {
                    ...
                    //判断是否是图片 暂时屏蔽，根据项目需要打开或额外处理
                    if(false && stripos($value,'https://xxxxx/upload/')!==false && (stripos($value,'.png')!==false || stripos($value,'.jpg')!==false || stripos($value,'.jpeg')!==false)){

                        $drawing = new Drawing();
                        
                        $imgstr = public_path().str_ireplace('https://wms.wmyth.com/','',$value);

                        $drawing->setName('image');
                        $drawing->setPath($imgstr);
                        $drawing->setHeight(160);
                        $drawing->setCoordinates($span . $column);
                        ...
                        $drawing->setWorksheet(self::$sheet);
                        self::$sheet->getRowDimension($column)->setRowHeight(160);
    ...
    public function excelSave($fileName = '',$suffix = 'xlsx',$path = '')
    {
        ...
        $root_path = runtime_path().'/'.$save_path;
    ...
    public function _import($filePath,array $sql,$where = [],$startRow = 1)
    {
        ...
        if(in_array($ext,['Xlsx','Xls'])){
            ...
            for ($row = $startRow; $row <= $row_count; $row++){
                ...
                $item = [];
                $one = [];
                //$one[] = [$sheet->getCell('A'.$row)->getValue() => $sheet->getCell('B'.$row)->getValue()];
                if(!empty($where)){
